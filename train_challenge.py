...
def freeze_layers(model: torch.nn.Module, num_layers: int = 0) -> None:
    ...
    data = list(model.named_parameters())
    for i in range(num_layers * 2):
        data[i][1].requires_grad = False
...
def train(
    ...
    axes = make_training_plot("Challenge Training")
...
def main() -> None:
    ...
    train(tr_loader, va_loader, te_loader, freeze_none, config("challenge.frozen_checkpoint").format(layer=0), 0)
    train(tr_loader, va_loader, te_loader, freeze_one, config("challenge.frozen_checkpoint").format(layer=1), 1)
    train(tr_loader, va_loader, te_loader, freeze_two, config("challenge.frozen_checkpoint").format(layer=2), 2)
    train(tr_loader, va_loader, te_loader, freeze_three, config("challenge.frozen_checkpoint").format(layer=3), 3)
