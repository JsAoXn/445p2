...
from model.source_challenge import Source_Challenge
...
def main() -> None:
    ...
    tr_loader, va_loader, te_loader, _ = get_train_val_test_loaders(
        task="source",
        batch_size=config("source_challenge.batch_size"),
    )

    # Model
    model = Source_Challenge()

    criterion = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=10e-3, weight_decay=0.01)

    print("Number of float-valued parameters:", count_parameters(model))

    # Attempts to restore the latest checkpoint if exists
    print("Loading source...")
    model, start_epoch, stats = restore_checkpoint(model, config("source_challenge.checkpoint"))

    axes = make_training_plot("Source_challenge Training")
    ...
    while curr_patience < patience:
        ...
        save_checkpoint(model, epoch + 1, config("source_challenge.checkpoint"), stats)
    ...
    plt.savefig(f"source_challenge_training_plot_patience={patience}.png", dpi=200)
    plt.ioff()
    plt.show()
